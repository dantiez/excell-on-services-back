...
namespace backend.Service {
    ...
    public class AuthService { 
        ...
        {
            var user = ctx.Users.SingleOrDefault(user => user.Active && user.Email == loginRequest.Email);
            if (user == null)
            {
        ...
        public async Task<LogoutReponse> LogoutAsync(int userId)
        {
            ...
            return new LogoutReponse { Success = false, Error = "Unable to logout user", ErrorCode = "L04" };
        }
        public async Task<SignUpResponse> SignupAsync(SignUpRequest signupRequest)
        {
            var existingUser = await ctx.Users.SingleOrDefaultAsync(user => user.Email == signupRequest.Email);
